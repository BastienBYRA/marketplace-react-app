...
const Marketplace = ({
  ...
  if (allCategories) {
    categoryElements = allCategories.map((category) => (
      <input
        className="marketplace-category-button"
        key={nanoid()}
        type="button"
        value={category.charAt(0).toUpperCase() + category.slice(1)}
        onClick={(e) => chooseCategory(e.target.value)}
      />
    ));
  }
  ...
  return (
    <div className="content">
      ...
      {productElements && (
        <div className="marketplace-category">{categoryElements}</div>
      )}
      {productElements && <div className="marketplace">{productElements}</div>}
      {!productElements && (
