...
const Basket = ({basket, emptyVariable, handleChangeBasket}) => {
    ...
    if(basket){
        ...
        basket.map(product => {
            totalBasketPrice = (product.quantity * product.product.price) + totalBasketPrice
        })

        totalBasketPrice = Math.round(((totalBasketPrice) + Number.EPSILON) * 100) / 100
    ...
    return (
        <div className='basket'>
            ...
            {basket.length > 0 && <div className='basket-separator'>
                        ...
                        <aside className='basket-total'>
                            <div className='basket-fixed'>
                                ...
                                <div className='basket-total-buttons'>
                                    <button className='basket-total-validate' onClick={
                                        () => handleChangeBasket(basket, null, 'VALIDATE_ORDER')
                                    }>Validate</button>
                                    <button className='basket-total-delete' onClick={
